...
def plot_signals(signals: dict[str, np.ndarray], cut_start: float = 0.0, cut_end: float = 16.0) -> None:
    """Plot one figure per signal for displacement and velocity."""
    t = signals["t"]
    disp = signals["disp"]
    vel = signals["vel"]

    t_disp, disp_cut = cut_signal(t, disp, cut_start, cut_end)
    t_vel, vel_cut = cut_signal(t, vel, cut_start, cut_end)

    fig_disp, ax_disp = plt.subplots(figsize=(12, 4))
    ax_disp.plot(t_disp, disp_cut, color="tab:blue", lw=1.1)
    ax_disp.set_title("Axial displacement")
    ax_disp.set_ylabel("Disp")
    ax_disp.set_xlabel("Time [s]")
    ax_disp.grid(True, linestyle=":", alpha=0.3)
    fig_disp.tight_layout()

    fig_vel, ax_vel = plt.subplots(figsize=(12, 4))
    ax_vel.plot(t_vel, vel_cut, color="tab:orange", lw=1.1)
    ax_vel.set_title("Axial velocity")
...
def main() -> None:
    ...
    plot_signals(signals, cut_start=cut_start, cut_end=cut_end)
