...
méthode de bissection déjà validée précédemment pour neuf tailles de dexel réparties autour
de la valeur de base retenue lors de l'étude de convergence de maillage préalable.
L'objectif est de déterminer si \(\apcritsim\) converge vers la
...
Il a été établi précédemment, pour une taille de dexel fixe \(\dxlsize = 2\times10^{-4}\)~m
(celle retenue lors de l'étude de convergence de maillage préalable), que la limite de
stabilité \(\apcritsim\) obtenue par bissection sur le
...
bissection que précédemment est appliquée indépendamment : un ensemble de simulations à
...
\begin{equation}
    ...
    \label{eq-eta}
\end{equation}
est la profondeur normalisée par la valeur théorique, \(\eta_-\) étant la valeur de
\(\eta\) du dernier cas simulé stable et \(\eta_+\) celle du premier cas simulé
instable. La bissection est poursuivie jusqu'à \(\eta_+ - \eta_- \leq 0,01\). La
limite simulée normalisée retenue est le centre de cet intervalle,
\(\etac = (\eta_- + \eta_+)/2\) ; l'intervalle \([\eta_-, \eta_+]\) lui-même
...
\begin{table}[H]
    ...
    valeur de base \(\dxlsize = 2\times10^{-4}\)~m retenue lors de l'étude de convergence de
    maillage préalable.}
    \label{tab-dexels}
    \begin{tabular}{@{}cccccccccc@{}}
        \toprule
        \(\dxlsize\) (\(\times10^{-5}\)~m) & 1,25 & 2,5 & 5 & 10 & 20 & 40 & 80 & 160 & 320 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \([\eta_-,\eta_+]\) et la référence théorique \(\eta=1\). L'axe vertical est
...
l'ordre de \SI{1}{\percent} à \SI{2}{\percent}. Le dexel de base retenu précédemment
...
Le même comportement qualitatif que celui observé précédemment est retrouvé ici : les
...
notable par rapport au cas précédent est que la convergence est atteinte rapidement du côté fin du
...
\SI{2}{\percent} sur la majeure partie du balayage, y compris au dexel de base retenu
précédemment. Raffiner davantage le dexel au-delà de cette plage ne se justifie donc pas au
...
ou dans les spécificités du modèle à profondeur variable.
