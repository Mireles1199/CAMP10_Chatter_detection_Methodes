...
Both modes use the **same effective window duration** T_win = T_lcm = 20 ms:
    ...
    by_modal:       N_modal_window  = 3   ->  T_win = 3 x 6.667 ms = 20 ms
                    step_modal      = 1   ->  hop   = T_modal       = 6.667 ms

    by_revolution:  N_rev_window    = 4   ->  T_win = 4 x 5 ms     = 20 ms
...
----------------
  ...
  F7 -- Summary table
  F8/F9 -- Same as F4/F5 for Lambda_k (log-likelihood ratio)
  F10 -- S_k vs segment index k (post-onset slope comparison)
"""
...
CFG_MODAL = {
    ...
    "param_mode": "by_modal",
    "params_physical": {
        "T_rev": _T_REV,  # informational only in by_modal
        "T_modal": _T_MOD,
        "N_modal_window": 3,  # T_win = 3 x 6.667 ms = 20 ms
...
CFG_REV = {
    ...
    "params_physical": {
        "T_rev": _T_REV,
        "N_rev_window": 4,  # T_win = 4 x 5 ms = 20 ms
...
def main() -> None:
    ...
    slope_m = np.polyfit(k_modal_post, S_modal_post, 1)[0] if k_modal_post.size > 1 else np.nan
    slope_r = np.polyfit(k_rev_post, S_rev_post, 1)[0] if k_rev_post.size > 1 else np.nan
    ...
    ax1.plot(rev["t"], rev["H"], marker=".", color=C_REV, label="by_revolution", alpha=0.75)
    ...
    ax2.plot(rev["t"], rev["S"], marker=".", color=C_REV, label="by_revolution", alpha=0.75)
    ax2.axhline(modal["b"], color=C_MODAL, linestyle="--", alpha=0.6, label=rf"$b$ modal = {modal['b']:.2f}")
    ax2.axhline(rev["b"], color=C_REV, linestyle="--", alpha=0.6, label=rf"$b$ rev = {rev['b']:.2f}")
    ...
    for row_y, t_mids, T_w, c, lbl in [
        (1.0, modal["t"], T_WIN_S, C_MODAL, f"by_modal       hop = {HOP_M_MS:.2f} ms"),
        (0.0, rev["t"], T_WIN_S, C_REV, f"by_revolution  hop = {HOP_R_MS:.2f} ms"),
    ]:
    ...
    ax3.set_title(f"Segment grid near $t_{{gt}}$  (T_win = {T_WIN_MS:.1f} ms,  zoom +/-{_WIN_S*1000:.0f} ms)")
    ax3.legend()

    # -- F4 -- Cross-correlation ----------------------------------------------
    fig4, (ax4a, ax4b) = plt.subplots(2, 1, figsize=fig_size(_SC, ncols=1))
    ax4a.plot(lags_ms, xcorr, color=C_MODAL)
    ax4a.axvline(lag_pk_ms, color=C_RED, linestyle="--", label=f"peak lag = {lag_pk_ms:.2f} ms")
    ...
    ax4b.axvline(lag_pk_ms, color=C_RED, linestyle="--", label=f"peak lag = {lag_pk_ms:.2f} ms")
    ax4b.axvline(0, color="gray", linestyle=":", label=f"Pearson r(0) = {r_zero:.4f}")
    ...
    fig6, axes6 = plt.subplots(1, 2, figsize=(fig_size(_SC)[0] * 2, fig_size(_SC)[1]))
    ...
    for ax_h, d, title_s in [
        ...
        ax_h.hist(d["H_free"], density=True, alpha=0.4, color=_CAZUL, bins="auto", label="$H$ stable")
        ax_h.hist(d["H_chat"], density=True, alpha=0.4, color=_CORANGE, bins="auto", label="$H$ chatter")
        ax_h.plot(x_pdf, _gauss(x_pdf, d["P0_mu"], d["P0_sig"]), color=_CVERDE,
                  label=rf"$P_0$: $\mu$={d['P0_mu']:.4f}, $\sigma$={d['P0_sig']:.4f}")
        ax_h.plot(x_pdf, _gauss(x_pdf, d["P1_mu"], d["P1_sig"]), color=_CRED,
    ...
    rows = {
        ...
        "Delta_t_d (ms)": [_fmt((td_modal - _T_GT) * 1e3, ".1f"), _fmt((td_rev - _T_GT) * 1e3, ".1f")],
    ...
    ax7.axis("off")
    tbl = ax7.table(cellText=df.values, rowLabels=df.index, colLabels=df.columns, loc="center", cellLoc="center")
    tbl.auto_set_font_size(True)
    tbl.auto_set_column_width(col=list(range(len(df.columns))))
    ax7.set_title(f"Summary -- Modal vs Revolution  (T_win = {T_WIN_MS:.0f} ms,  T_lcm = 20 ms)", pad=12)
    fig7.tight_layout()
    ...
    ax8a.axvline(lag_L_ms, color=C_RED, linestyle="--", label=f"peak lag = {lag_L_ms:.2f} ms")
    ...
    ax8b.axvline(lag_L_ms, color=C_RED, linestyle="--", label=f"peak lag = {lag_L_ms:.2f} ms")
    ...
    ax9.set_title(f"Option A -- $\\Lambda_{{\\rm modal}}$ vs $\\Lambda_{{\\rm rev}}$  (r = {r_zero_L:.4f})")
    ax9.legend()
    ax9.set_aspect("equal", "box")

    # -- F10 (Option B) -- S vs segment index k (post-onset) ------------------
    fig10, ax10 = plt.subplots(figsize=fig_size(_SC))
    ax10.plot(k_modal_post, S_modal_post, marker=".", color=C_MODAL, label=f"by_modal  slope = {slope_m:.4f}/seg")
    ...
    if not np.isnan(slope_m):
        ax10.plot(k_modal_post, np.polyval(np.polyfit(k_modal_post, S_modal_post, 1), k_modal_post),
                  color=C_MODAL, linestyle="--", alpha=0.5)
    if not np.isnan(slope_r):
        ax10.plot(k_rev_post, np.polyval(np.polyfit(k_rev_post, S_rev_post, 1), k_rev_post),
                  color=C_REV, linestyle="--", alpha=0.5)
    ax10.axhline(modal["b"], color=C_MODAL, linestyle=":", alpha=0.6, label=rf"$b$ modal = {modal['b']:.2f}")
    ax10.axhline(rev["b"], color=C_REV, linestyle=":", alpha=0.6, label=rf"$b$ rev = {rev['b']:.2f}")
    ax10.ticklabel_format(style="sci", axis="y", scilimits=(0, 0))
    ax10.set_title(r"Option B -- $S_k$ vs segment index $k$  (post-onset region $t > t_{gt}$)")
