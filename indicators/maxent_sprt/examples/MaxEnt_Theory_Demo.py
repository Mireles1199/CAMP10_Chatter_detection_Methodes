...
=====================
Nine theoretical demonstration figures for the MaxEnt-SPRT indicator:
  ...
  D6 -- ln p0(H), ln p1(H) vs H  (anatomy of Lambda(H))
  D7 -- p0(H(t)), p1(H(t)) over time  (which distribution wins each segment)
  D8 -- ln p0(H(t)), ln p1(H(t)) over time  (log-likelihoods and Lambda_k)
  D9 -- Both PDFs: theory + real histogram (goodness of fit)
"""
...
# =============================================================================
# DATA / CONFIG  (same file used in all other examples)
# =============================================================================
...
def main() -> None:
    ...
    cfg_no_reset = copy.deepcopy(base_cfg)
    cfg_no_reset["params_physical"]["reset_on_H0"] = False

    # =========================================================================
    # RUN  (two variants needed: reset=True and reset=False)
    # =========================================================================
    print("Running with reset=True  (pre-run) ...")
    res_reset = run_maxent_sprt(sig, base_cfg)

    # ── Resolve β according to BETA_MODE ─────────────────────────────────────
    if BETA_MODE == "classical":
        meta_pre = res_reset.meta or {}
        p0_mu_pre = meta_pre["P0_mu"]
        p0_sig_pre = meta_pre["P0_sigma"]
        p1_mu_pre = meta_pre["P1_mu"]
        p1_sig_pre = meta_pre["P1_sigma"]
        n_sig_pre = norm.isf(_ALPHA)  # ≈ 3.0
        h_thr_pre = p0_mu_pre + n_sig_pre * p0_sig_pre
        beta = float(norm.cdf(h_thr_pre, p1_mu_pre, p1_sig_pre))  # β_cl
        print(f"  BETA_MODE='classical': β_cl = {beta:.6f}  (α = {_ALPHA:.6f})")
        base_cfg["params_physical"]["beta"] = beta
        cfg_no_reset = copy.deepcopy(base_cfg)
        cfg_no_reset["params_physical"]["reset_on_H0"] = False
    ...
    res_noreset = run_maxent_sprt(sig, cfg_no_reset)
    ...
    mask_lp = (d_reset["t"] > _T_GT) & (d_reset["Lambda"] >= b_val)
    td_lambda_puntual = float(d_reset["t"][mask_lp][0]) if mask_lp.any() else np.nan
    ...
    # For unequal sigmas this is the root of the quadratic Λ(H)=0. Numerically:
